...
def align_unknown():
    ...
    if os.path.exists(un_vec) is False:
        ...
        result, length, dirv= write_vector_csv(user)
        counter1 = 0
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not os.path.exists(origin):
            ...
            r_s = time.time()
            ret, frame_t = video_capture.read()           # 從攝影機拍一張照片
            ...
            while (1):
                ...
                # cv2.putText(影像,   文字, 座標,     字型, 大小,        顏色, 線條寬度, 線條種類#可省略)
                cv2.putText(  frame, Intro, (40, 40), font,  0.5, (255, 0, 0),        0                 )
                if ret:
                ...
                if cv2.waitKey(50) & 0xFF == ord('a'):
                    while record <= 500:
                        ...
                        cv2.putText(  frame, Intro1, (40,40), font, 0.5, (255, 0, 0), 0      )
                        if ret:
                            cv2.imshow('Capture', frame)
                        cv2.waitKey(30)
    ...
    result,length,dirv = write_vector_csv(person)
    counter1 =0
